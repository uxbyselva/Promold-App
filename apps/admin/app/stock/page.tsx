...
export default async function StockPage() {
  ...
  const [{ data: items }, { data: locations }, { data: levels }, { data: packs }, { data: recent }] =
    await Promise.all([
      supabase
        .from('inventory_items')
        .select('id, sku, name, category, unit_of_measure, consumption_mode, min_level, reorder_quantity, average_cost')
        .eq('is_active', true)
        .is('deleted_at', null)
        .order('name'),
      ...
      supabase
        .from('open_packs')
        .select('pack_id, item_id, sku, name, location_id, location_name, opened_at, opened_by, unit_cost, jobs_served')
        .order('opened_at'),
      ...
      supabase
        ...
        .select('id, item_id, kind, from_location_id, to_location_id, quantity, job_id, reason, created_at, created_by')
    ...
    <OfficeShell session={session} mode="office">
      <div className="page-head">
        <div>
          <h1>Stock</h1>
          <p>
            Levels are derived from the movement ledger and are never typed in. A level somebody
            can edit is a level nobody trusts by month three.
          </p>
        </div>
