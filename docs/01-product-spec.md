...
3. **Nobody knows which jobs made money.** Jobs are billed at a flat quoted
   price, but labour hours, materials, mileage and equipment days are never
   totalled against that price.

The app exists to fix those three things. Everything else is in service of
them.
...
A **Job** is the unit of billing and the unit of costing. It carries a quoted
price and may span multiple days.
...
### 4.4 Material purchase requests
...
For each job, against its quoted price:
...
Margin = quoted price − total cost. This report is the payoff for all the
logging the field crew is asked to do; without it, the logging reads as
surveillance. Build it in the same phase as the logging it depends on.

### 4.9 Compliance and documents
...
- Document store per job and per site: scope of work, insurance claim number
  and adjuster contact, lab results, SDS sheets.
...
- Invoicing and payment collection — export to the accounting package
  instead.
- Estimating / quoting workflow. A job carries a quoted price entered by
  hand; building the estimate is Phase 3.
